...
class AnimalFinderScreen extends ConsumerStatefulWidget {
  const AnimalFinderScreen({super.key});

  @override
  ConsumerState<AnimalFinderScreen> createState() =>
      _AnimalFinderScreenState();
}

class _AnimalFinderScreenState extends ConsumerState<AnimalFinderScreen> {
  ...
  void _scheduleAutoAdvance() {
    ...
    _autoAdvanceTimer = Timer(
      ...
      () {
        if (mounted) {
          ref.read(animalFinderProvider.notifier).nextQuestion();
        }
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    // Schedule auto-advance the moment a selection is registered
    ref.listen<AsyncValue<AnimalFinderState>>(animalFinderProvider,
        (prev, next) {
    ...
    final gameAsync = ref.watch(animalFinderProvider);
            ...
            gameAsync.when(
              ...
              data: (state) => state.isFinished
                  ? _FinishedView(
                      ...
                      total: state.total,
                      onPlayAgain: () =>
                          ref.read(animalFinderProvider.notifier).restart(),
                      onBack: () => context.pop(),
                      copy: copy,
                    )
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final language =
        ref.watch(appLanguageControllerProvider).valueOrNull ??
            AppLanguage.spanish;
    final animalName = question.correctWord.localizedTitle(language);

    final prompt = state.showCelebration
        ? copy.greatJob
        : copy.whereIsThe(animalName);
...
// ─────────────────────────────────────────────
// Glass helpers (shared visual style)
// ─────────────────────────────────────────────
