...
│               └── widgets/  # CustomPainter, overlays, componentes locales
└── shared/               # providers/modelos cruzados entre features (words, language, services)
```

## Patrón por feature

1. **Models**: clases inmutables (`copyWith`) que representan el estado del dominio del feature (ej. `TracePathShape`, `DrawStroke`).
2. **Providers**: un `State` inmutable + `Notifier`/`AsyncNotifier` que expone acciones mutadoras; se registra como `NotifierProvider`/`AsyncNotifierProvider` global.
3. **Presentation**: `ConsumerWidget`/`ConsumerStatefulWidget` que hace `ref.watch`/`ref.read` del provider del feature; efectos visuales (partículas, curvas, trazos) se aíslan en `CustomPainter` dentro de `widgets/`.
4. **Routing**: cada pantalla nueva se registra en `app/router/app_router.dart`; para minijuegos, el `switch` sobre `gameId` mapea el `id` del catálogo (`app/constants/mini_game_data.dart`) a su `Screen`.
...
- [[Mini Juegos]] — catálogo y detalle de cada minijuego, incluye `trace_path` y `free_draw`.
